...
class TestTodoAPI:
    ...
    def test_create_todo_database_error(self, client):
...
class TestIntegration:
    def test_complete_todo_lifecycle(self, client):
        create_response = client.post('/api/todos', json={'title': 'Integration Test'})
        ...
        assert update_response.get_json()['data']['completed'] is True
    ...
    def test_multiple_todos_workflow(self, client):
        for i in range(5):
            client.post('/api/todos', json={'title': f'Todo {i+1}'})
        response = client.get('/api/todos')
        todo_id = response.get_json()['data'][0]['id']
        client.put(f'/api/todos/{todo_id}', json={'completed': True})
        client.delete(f'/api/todos/{todo_id}')
        assert client.get('/api/todos').get_json()['count'] == 4
